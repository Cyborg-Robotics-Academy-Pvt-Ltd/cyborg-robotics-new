...
export const HangingIdCard = ({
  ...
  const onPointerDown = useCallback(
    (e: React.PointerEvent) => {
      // Skip drag if the user clicked an interactive element inside the card
      const target = e.target as HTMLElement;
      const interactive = target.closest(
        "button, input, label, a, textarea, select, [role='button']",
      );
  ...
  const onPointerUp = useCallback(
    (e: React.PointerEvent) => {
      ...
      // Release into physics after drag
      startPhysics();
    },
    [startPhysics],
  );
  ...
  return (
    ...
    >
      ...
      {/* Pendulum assembly — drag target is the rope+card wrapper */}
      <div
        className={cn(
      ...
      >
        ...
        {/* 
          ID Card wrapper:
          - pointer-events: auto so clicks on children (photo, edit btn, etc.) still fire
          - onClick on the wrapper triggers the physics impulse only when clicking
        ...
        */}
        ...
        >
          {children ?? (
            // Default fallback card (shown only when no children passed)
