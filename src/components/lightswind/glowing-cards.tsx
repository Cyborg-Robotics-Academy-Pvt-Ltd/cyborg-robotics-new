...
export interface GlowingCardProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  glowColor?: string;
  // Removed unused prop: hoverEffect
}
...
export interface GlowingCardsProps {
  ...
  // Removed unused prop: enableHover
}
...
export const GlowingCards: React.FC<GlowingCardsProps> = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Removed unused state update: setMousePosition({ x, y });
      setShowOverlay(true);
            ...
            >
              {React.Children.map(children, (child) => {
                if (React.isValidElement(child) && child.type === GlowingCard) {
                  // Cast child to ReactElement with GlowingCardProps for type safety
                  const typedChild =
